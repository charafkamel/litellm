    ...
    try:
        for info in socket.getaddrinfo(host, None):
            ...
            if addr in _BLOCKED_METADATA_HOSTS or ipaddress.ip_address(addr).is_link_local:
                raise ValueError(f"Compresr guardrail api_base {host!r} resolves to blocked address {addr!r}")
...
class CompresrGuardrail(CustomGuardrail):
        ...
        if raw_response is None or not 200 <= raw_response.status_code < 300:
            self._handle_compress_failure(
                "Compresr compression service returned an error",
                {
                    ...
                    "body": (getattr(raw_response, "text", "") or "")[:500],
                },
            )
            return None
    ...
    async def apply_guardrail(
        ...
        # Recovery needs a stable, framework-issued call id to scope stored
        # originals to this request (see _resolve_call_id). Without one we
        # cannot serve originals back safely, so we compress without markers.
        call_id = _resolve_call_id(logging_obj)
        recovery_enabled = self.enable_retrieval and call_id is not None
        ...
        if not recovery_enabled or not originals or call_id is None:
